    ...
    , ui(new Ui::simple)
{
    ...
    // Create a content widget to hold the images
    QWidget* contentWidget = new QWidget();
    QVBoxLayout* contentLayout = new QVBoxLayout(contentWidget);

    // Load images from the folders
    loadImagesFromFolder("1", contentLayout);
...
void simple::loadImagesFromFolder(const QString& folderName, QVBoxLayout* layout) {
    ...
    for (const QString& file : imageFiles) {
        ...
        imageLabel->setPixmap(pixmap.scaled(2000, 1200, Qt::KeepAspectRatio, Qt::SmoothTransformation));
